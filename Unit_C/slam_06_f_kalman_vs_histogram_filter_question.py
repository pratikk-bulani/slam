...
class Density:
    def __init__(self, mu, sigma2):
        self.mu = float(mu)
        self.sigma2 = float(sigma2) # Variance i.e. sigma^2
...
def kalman_filter_step(belief, control, measurement):
    ...
    # Correction.
    c = 1
    kalman_gain = c*prediction.sigma2 / (c**2 * prediction.sigma2 + measurement.sigma2)
    mean = prediction.mu + kalman_gain*(measurement.mu-c*prediction.mu)
    variance = (1-kalman_gain*c)*prediction.sigma2
    correction = Density(mean, variance)  # Replace
