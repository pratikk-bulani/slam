...
class ExtendedKalmanFilterSLAM:
    ...
    def predict(self, control):
        ...
        self.state = append(self.g(self.state[:3], control, self.robot_width), old_state[3:], axis = -1)
    ...
    def add_landmark_to_state(self, initial_coords):
        ...
        self.state = append(self.state, initial_coords, axis = -1)
        new_covar = eye(len(self.covariance)+2)
        new_covar[:-2, :-2] = self.covariance
        new_covar[-2, -2] = 1e10
        new_covar[-1, -1] = 1e10
        self.covariance = new_covar
    ...
    def correct(self, measurement, landmark_index):
        ...
        H = zeros((2, 3+2*self.number_of_landmarks))  # Replace this.
        H[:, :3] = H3
        H[:, 3+2*landmark_index:3+2*landmark_index+2] = -H3[:, :2]
